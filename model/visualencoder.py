...
class ResNetLayer(nn.Module):
    ...
    def forward(self, inputBatch):
        ...
        if self.stride == 1 and inputBatch.shape[1] == batch.shape[1]:
...
class visualFrontend(nn.Module):
    ...
    def forward(self, inputBatch):
        ...
        batchsize, T, C, H, W = batch.shape
        batch = batch.reshape(batchsize*T, C, H, W)
        outputBatch = self.resnet(batch)
        outputBatch = outputBatch.reshape(batchsize, T, 512)
        outputBatch = outputBatch.transpose(1, 2)
        return outputBatch

class visualFrontend_Freq(nn.Module):
    """
    A visual feature extraction module. Generates a 512-dim feature vector per video frame.
    Architecture: A 3D convolution block followed by an 18-layer ResNet.
    """
    ...
    def __init__(self):
        super(visualFrontend_Freq, self).__init__()
        self.frontend3D = nn.Sequential(
                            nn.Conv3d(12, 64, kernel_size=(5,7,7), stride=(1,2,2), padding=(2,3,3), bias=False),
                            nn.BatchNorm3d(64, momentum=0.01, eps=0.001),
                            nn.ReLU(),
                            nn.MaxPool3d(kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1))
                        )
...
class VisualEncoderModel(nn.Module):
    def __init__(self):
        ...
        self.freqenceFrontend = visualFrontend_Freq()
        self.visualTCN = visualTCN()  # Visual Temporal Network TCN
        self.freqenceTCN = visualTCN()
        self.visualConv1D = visualConv1D()  # Visual Temporal Network Conv1d
        return

    def forward(self, x, f):
        x = self.visualFrontend(x)  # (1, 3, 512, 96, 96) -> (1, 512, 512)
        x = self.visualTCN(x)       # (1, 512, 512) -> (1, 512, 512)
        f = self.freqenceFrontend(f)# (1, 12, 512, 96, 96) -> (1, 512, 512)
        f = self.freqenceTCN(f)
        x = torch.concat((x, f), dim=1)
        x = self.visualConv1D(x)    # (1, 1024, 512) -> (1, 256, 512)
        return x
...
if __name__ == '__main__':
    ...
    input_tensor1 = torch.randn(B, C1, T, H, W).to(device)
    input_tensor2 = torch.randn(B, C2, T, H, W).to(device)
    model = VisualEncoderModel().to(device)
    print(f"输入张量的形状: {input_tensor1.shape, input_tensor2.shape}")
