...
class FakeAVDataset(Dataset):
    ...
    def get_label(self, meta: Metadata) -> Tensor:
        ...
        category = meta.file.split('/')[-5]
        country  = meta.file.split('/')[-4]
        gender   = meta.file.split('/')[-3]
        identity = meta.file.split('/')[-2]
        file_name = meta.file.split("/")[-1].split(".")[0] + ".npy"
        # root_path + {RealVideo-RealAudio/African/men/id00166} + 00010.npy
        path = os.path.join(self.root, "label", category, country, gender, identity, file_name)
    ...
    def get_label_with_match_scores(self, meta: Metadata) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        gender   = meta.file.split('/')[-3]
        identity = meta.file.split('/')[-2]
        Path(os.path.join(self.root, "label", category, country, gender, identity)).mkdir(parents=True, exist_ok=True)
        Path(os.path.join(self.root, "match_scores", category, country, gender, identity)).mkdir(parents=True, exist_ok=True)

        boundary_map_file_name = meta.file.split("/")[-1].split(".")[0] + ".npy"
        boundary_map_file_path = os.path.join(self.root, "label", category, country, gender, identity, boundary_map_file_name)

        match_scores_file_name = meta.file.split("/")[-1].split(".")[0] + ".npz"
        match_scores_file_path = os.path.join(self.root, "match_scores", category, country, gender, identity, match_scores_file_name)
